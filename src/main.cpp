...
#if defined(NDEBUG)
# error "Altbit cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8f33543cec6536b79d645599b0d2e16114a25bc9ac3e990e3dc331d9f0d3738c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Altbit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Altbit Signed Message:\n";
...
{
    // Altbit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Altbit: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Altbit: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Altbit: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Altbit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Altbit: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8f33543cec6536b79d645599b0d2e16114a25bc9ac3e990e3dc331d9f0d3738c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is AltBit [ATBI]";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("975284710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1504390157;
        ...
        assert(block.hashMerkleRoot == uint256("0x29208688c920186fde936303e592fa56b1b3934087babc81fe233e7c1f6a7430"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Altbit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AltbitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AltbitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AltbitMiner(CWallet *pwallet)
{
    printf("AltbitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("altbit-miner");
    ...
    try { loop {
        ...
        printf("Running AltbitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AltbitMiner, pwallet));
}
